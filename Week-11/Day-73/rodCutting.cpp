vector<int> dfs(int start,int end,vector<int> &B,long long &cost, map<pair<int,int>, pair<long long,vector<int>>> &dp)
{
    vector<int> ans;
    int m = B.size();
    ...
    for(int i=0;i<m;i++)
    {
        ...
        int temp = B[i];
        B[i] = INT_MIN;
        rarr=dfs(temp,end,B,r,dp);
        larr=dfs(start,temp,B,l,dp);
        B[i] = temp;
        if(cost > l+r)
        {
            cost = l+r;
...
vector<int> Solution::rodCut(int A, vector<int> &B) {
    ...
    vector<int> ans = dfs(0,A,B,cost,dp);
